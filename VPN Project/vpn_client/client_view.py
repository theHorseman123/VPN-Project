...
class MyTabView(ctk.CTkTabview):
    ...
    def create_proxy_frame(self, addr, status):
        ...
        status_icon = "unlocked.png" if status == "0" else "locked.png"
...
def main():
    parser = argparse.ArgumentParser(description='This is the code for the VPN client')
    parser.add_argument("-i" , type=str, help="the interface the client will run from")
    ...
    try:
        App(server_address=server_addr, interface=args.i)
    except Exception as error:
        disable_proxy()
    disable_proxy()
