...
class App:
    ...
    def create_widgets(self):
        self.label_status = ctk.CTkLabel(self.root, text="Status: Disconnected", text_color="red", bg_color="#404B56")
        self.label_status.grid(row=0, column=0, pady=10, sticky="n")

        self.proxy_list_frame = ctk.CTkFrame(self.root, bg_color="#404B56")
        self.proxy_list_frame.grid(row=1, column=0, sticky="nsew")

        self.canvas_frame = ctk.CTkFrame(self.root, bg_color="#404B56")
    ...
    def populate_proxy_list(self):
        canvas = Canvas(self.proxy_list_frame, bg="#404B56")
        scrollbar = Scrollbar(self.proxy_list_frame, command=canvas.yview)
        scrollable_frame = ctk.CTkFrame(canvas, bg_color="#404B56")
        ...
        for proxy in proxies:
            frame = ctk.CTkFrame(scrollable_frame, bg_color="#404B56")
            frame.pack(fill="x", pady=5, padx=5)

            label_ip = ctk.CTkLabel(frame, text=f"IP: {proxy['ip']}", bg_color="#404B56")
            ...
            label_icon = ctk.CTkLabel(frame, text="", image=icon_photo, bg_color="#404B56")
            label_icon.image = icon_photo
            label_icon.pack(side="left", padx=5)

            button_connect = ctk.CTkButton(frame, text="Connect", command=lambda ip=proxy['ip']: self.connect_vpn(ip), bg_color="#404B56")
            button_connect.pack(side="right", padx=5)

            proxy["label_status"] = ctk.CTkLabel(frame, text="", bg_color="#404B56")
